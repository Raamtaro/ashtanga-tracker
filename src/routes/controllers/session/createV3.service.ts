...
export type SessionV3DbClient = Prisma.TransactionClient | PrismaClient;
...
export type ResolvedCandidateCard = {
...
export type CandidateCardResponse = Omit<ResolvedCandidateCard, "poseId" | "isTwoSided">;

export type CandidatePoseOption = {
    slug: string;
    sanskritName: string;
    englishName: string | null;
    sequenceGroup: SequenceGroup;
...
export function validateScoredPosesSubset(scoredPoses: string[] | undefined, validSlugs: string[]) {
...
export function toCandidateCardResponse(card: ResolvedCandidateCard): CandidateCardResponse {
    const { poseId: _poseId, isTwoSided: _isTwoSided, ...responseCard } = card;
    return responseCard;
}

export function parseCandidateResolutionError(error: unknown) {
...
export async function buildSessionWithScoreCards(
    params: {
